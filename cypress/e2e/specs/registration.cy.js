...
describe("example tests", () => {
  ...
  it("navigate to login form", () => {
    cy.get('[data-qa="continue-button"]').should("be.visible").click()
    cy.get("a[href=/logout").should("be.visible").click()
    cy.get("a[href=/login").should("be.visible").click()
    cy.get('data-pa="login-email"]').clear().type(email)
    cy.get('data-pa="login-password"]').check().type("test123")
    cy.get('data-pa="login-button"]').should("be.visible").click()
    cy.get("a").contains("Logged in as Emir")
